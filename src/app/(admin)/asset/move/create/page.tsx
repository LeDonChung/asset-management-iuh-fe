...
export default function MoveCreatePage() {
  ...
  useEffect(() => {
    if (isAddAssetModalOpen) {
      ...
      savedRoomIdRef.current = selectedRoomId;
    }
  }, [isAddAssetModalOpen]);

  useEffect(() => {
    if (!isAddAssetModalOpen && (savedUnitIdRef.current || savedRoomIdRef.current)) {
      if (savedUnitIdRef.current && savedUnitIdRef.current !== selectedUnitId) {
        setSelectedUnitId(savedUnitIdRef.current);
      }
      if (savedRoomIdRef.current && savedRoomIdRef.current !== selectedRoomId) {
        setSelectedRoomId(savedRoomIdRef.current);
      }
      setTimeout(() => {
        savedUnitIdRef.current = "";
        savedRoomIdRef.current = "";
      }, 100);
    }
  }, [isAddAssetModalOpen]);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-300 mb-6 ">
          ...
          <Table<Asset>
            key={`assets-table-${selectedAssetsForMove.length}`}
            columns={columns}
