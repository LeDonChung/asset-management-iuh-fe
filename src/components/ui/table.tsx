...
export function Table<T = any>({
    ...
    const handleSort = (columnKey: string) => {
        ...
        const existingSortIndex = currentSortConfigs.findIndex(
            (config) => config.field === columnKey
        );
        ...
        if (multiSort) {
            ...
            if (existingSortIndex >= 0) {
                ...
                if (existingSort.direction === "asc") {
                    ...
                    // Remove this sort
                    newSortConfigs = currentSortConfigs.filter(
                        (config) => config.field !== columnKey
                    );
                }
            } else {
                ...
                newSortConfigs = [
                    ...currentSortConfigs,
                    {
                        field: columnKey,
                        direction: "asc",
                        priority: currentSortConfigs.length,
                    },
                ];
            }
        } else {
            ...
            if (existingSortIndex >= 0) {
                ...
                if (existingSort.direction === "asc") {
                    newSortConfigs = [
                        { field: columnKey, direction: "desc", priority: 0 },
                    ];
                } else {
                    newSortConfigs = [];
                }
    ...
    const sortedData = useMemo(() => {
        ...
        return [...data].sort((a, b) => {
            ...
            const sortedConfigs = [...currentSortConfigs].sort(
    ...
    const getSortIcon = (columnKey: string) => {
        ...
        const priority =
            multiSort && currentSortConfigs.length > 1
                ? sortConfig
                    ? (sortConfig.priority || 0) + 1
                    : null
                : null;
    ...
    const isAllSelected =
        displayData.length > 0 && selectedRowKeys.length === displayData.length;
    ...
    const handleSelectAll = () => {
        if (isAllSelected) {
            ...
            const allKeys = displayData.map((record, index) =>
                                    ...
                                    return rangeWithDots.map((page, index) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`min-w-[36px] px-3 py-2 text-sm rounded ${currentPage === page
                        ...
                        <tr>
                            ...
                            {columns.map((column, index) => {
                                ...
                                return (
                                    <th
                                        key={column.key}
                                        className={`px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider relative ${column.sortable ? "cursor-pointer hover:bg-gray-100" : ""
                                            } ${column.className || ""}`}
                                        style={{
                                    ...
                                    {columns.map((column) => (
                                        ...
                                        >
                                            <div
                                                className="truncate"
                                                title={
                                                    typeof renderCell(column, record, index) === "string"
